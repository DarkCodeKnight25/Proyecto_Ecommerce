...
const productoModel = {    
     crearProducto: async(nombre, idproveedor, marca, modelo, color, costo, precio, stock, estado, descripcion, imagen) => {
        let resultado = null;
        try {
            resultado = await producto.create({ nombre, idproveedor, marca, modelo, color, costo, precio, stock, estado, descripcion, imagen });
    ...
    actualizarProducto: async(id, nombre, idproveedor, marca, modelo, color, costo, precio, stock, estado, descripcion) => {
        let resultado = null;
        try {
            resultado = await producto.findByPk(id);
            if(resultado!=null){
                resultado = await resultado.update({ nombre, idproveedor, marca, modelo, color, costo, precio, stock, estado, descripcion });
            }       
            console.log(resultado);
        } catch (exception) {
    ...
    listarProductos: async() => {
        try {
            const resultados = producto.findAll();
    ...
    listarProductosActivos: async() => {
        try {
            const consulta = "SELECT * FROM producto WHERE estado = :estado";
    ...
    obtenerProducto: async(id) => {
        ...
        try {
            [resultado] = await  producto.findAll({
                attributes: ['id', 'nombre', 'idproveedor', 'marca', 'modelo', 'color', 'costo', 'precio', 'stock','estado','descripcion'], // Selecciona los campos que deseas obtener
                where: {
